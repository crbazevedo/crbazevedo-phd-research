#!/usr/bin/env python3
"""
Enhanced FTSE ASMS-EMOA Experiment - Async Version
With multiprocessing, better resource management, and fixed expected hypervolume calculation
"""
...
# Async and multiprocessing imports
import asyncio
...
class AsyncDecisionMaker:
    """Async decision maker with proper expected hypervolume calculation"""
...
class AsyncHvDM(AsyncDecisionMaker):
    """Async Hypervolume Decision Maker with fixed expected hypervolume calculation"""
...
class AsyncRDM(AsyncDecisionMaker):
    """Async Random Decision Maker"""
    ...
    def _calculate_expected_hypervolume(self, solution: Solution, 
                                      ...
                                      h: int) -> float:
        """Calculate expected hypervolume for a single solution"""
        base_hv = getattr(solution, 'hypervolume_contribution', 0.1)
        predicted_state = kalman_pred.get('state', np.array([0.1, 0.05]))
        predicted_weights = dirichlet_pred.get('mean_prediction', 
                                             np.ones(len(solution.P.investment)) / len(solution.P.investment))
...
class AsyncMDM(AsyncDecisionMaker):
    """Async Median Decision Maker"""
...
def load_ftse_data_async(start_date: str, end_date: str) -> pd.DataFrame:
    """Load FTSE data from existing CSV files - async version"""
...
def calculate_coherence_async(portfolio_weights: np.ndarray, 
...
def calculate_transaction_costs_async(current_weights: np.ndarray, 
...
def run_single_experiment_worker(args):
    ...
    (run_id, returns_data, k_values, h_values, historical_days, stride_days) = args
    
    logger.info(f"Starting run {run_id + 1}")
        ...
        for h in h_values:
            ...
            for period in range(n_periods):
                ...
                Portfolio.robust_covariance = historical_data.cov().values
                                    ...
                                    # Initialize SMS-EMOA with better reference points for financial data
                    algorithm_params = {
                        'population_size': 100,
                        'generations': 40,
                        'tournament_size': 2,
                ...
                for dm in decision_makers:
                    ...
                    if current_weights is not None:
                        transaction_cost = calculate_transaction_costs_async(
                    ...
                    coherence = calculate_coherence_async(portfolio_weights, population_weights)
    ...
    logger.info(f"Completed run {run_id + 1}")
    return run_results

class AsyncFTSEExperiment:
    """Async FTSE experiment with multiprocessing and better resource management"""
    
    def __init__(self, n_runs: int = 10, max_workers: int = None):
        self.n_runs = n_runs
        self.max_workers = max_workers or min(mp.cpu_count(), 4)  # Limit CPU usage
        self.results = {}
        
    async def run_async_experiment(self):
        """Run the async experiment with multiprocessing"""
        logger.info(f"Starting Async FTSE ASMS-EMOA Experiment with {self.max_workers} workers")
        ...
        # Load data
        returns_data = load_ftse_data_async(start_date, end_date)
        
        # Prepare arguments for multiprocessing
        args_list = [(run_id, returns_data, k_values, h_values, historical_days, stride_days) 
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            # Submit all tasks
            future_to_run = {executor.submit(run_single_experiment_worker, args): args[0] 
            ...
            for future in as_completed(future_to_run):
                ...
                try:
                    ...
                    logger.info(f"Received results from run {run_id + 1}")
                except Exception as exc:
                    logger.error(f"Run {run_id + 1} generated an exception: {exc}")
        ...
        logger.info("Async experiment completed!")
    ...
    def generate_comprehensive_report(self):
        ...
        report = []
        report.append("# Async FTSE ASMS-EMOA Experiment Report")
        ...
        # Save report
        with open('async_ibovespa_experiment_report.md', 'w') as f:
            f.write('\n'.join(report))
        
        logger.info("Comprehensive report generated: async_ibovespa_experiment_report.md")

    def create_comprehensive_visualizations(self, all_run_results: List[Dict]):
        """Create comprehensive visualizations"""
        # Create output directory
        os.makedirs('async_ibovespa_results', exist_ok=True)
        ...
        logger.info("Comprehensive visualizations saved to async_ibovespa_results/")
    ...
    def _plot_wealth_accumulation(self, all_run_results: List[Dict]):
        ...
        plt.tight_layout()
        plt.savefig('async_ibovespa_results/wealth_accumulation_curves.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_performance_comparison(self):
        ...
        plt.tight_layout()
        plt.savefig('async_ibovespa_results/performance_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_expected_hypervolume_analysis(self):
        ...
        plt.tight_layout()
        plt.savefig('async_ibovespa_results/expected_hypervolume_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
...
async def main():
    ...
    experiment = AsyncFTSEExperiment(n_runs=10, max_workers=4)  # Limit to 4 workers
    await experiment.run_async_experiment()
