#!/usr/bin/env python3
"""
Enhanced FTSE ASMS-EMOA Experiment V2
With multiple runs, proper wealth tracking, and fixed expected hypervolume calculation
"""
...
class EnhancedDecisionMaker:
    ...
    def __init__(self, name: str):
        ...
        self.wealth_accumulation = []  # Track wealth over time
...
class EnhancedHvDM(EnhancedDecisionMaker):
    ...
    def select_portfolio(self, pareto_frontier: List[Solution], 
        ...
        if selected_solution is not None:
            ...
            conditional_hv = n_step_predictor.compute_conditional_expected_hypervolume(
                pareto_frontier, selected_solution, kalman_predictions, dirichlet_predictions, h)
            
            return selected, {
                'expected_hypervolume': expected_hv_dist,
...
class EnhancedFTSEExperimentV2:
    """Enhanced FTSE experiment with multiple runs and proper tracking"""
    
    def __init__(self):
        self.n_runs = 10  # Multiple runs for statistical significance
        self.results = {}
        self.n_step_predictor = NStepPredictor(max_horizon=3)
        self.benchmark_calculator = BenchmarkCalculator()
        
    def load_ftse_data(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        if len(csv_files) == 0:
            logger.error(f"No CSV files found in {ftse_data_path}")
            logger.error(f"Directory exists: {os.path.exists(ftse_data_path)}")
            if os.path.exists(ftse_data_path):
                logger.error(f"Directory contents: {os.listdir(ftse_data_path)}")

        # Load and combine data from multiple files to simulate multiple assets
        all_data = []
        asset_names = []

        for i, file_path in enumerate(csv_files[:30]):  # Use first 30 files as assets
            try:
                # Read CSV file
    ...
    def calculate_coherence(self, portfolio_weights: np.ndarray, 
                          population_weights: List[np.ndarray]) -> float:
        """Calculate coherence using cosine similarity (Eq 7.14)"""
        ...
        # Calculate cosine similarity between portfolio and centroid
    ...
    def calculate_transaction_costs(self, current_weights: np.ndarray, 
        ...
        # Transaction cost rate (0.1% per weight change)
    ...
    def run_single_experiment(self, run_id: int, returns_data: pd.DataFrame,
        ...
        for k in k_values:
            for h in h_values:
                ...
                current_wealth = 100000.0  # R$ 100k initial investment
                ...
                for period in range(n_periods):
                    ...
                    future_end = min(end_idx + 60, len(returns_data))  # 60-day future window
                    ...
                    algorithm_params = {
                        ...
                        'crossover_rate': 0.8,
                        'mutation_rate': 0.1,
                        'reference_point_1': 0.0,
                        'reference_point_2': 1.0
                    }
                    ...
                    for horizon in [1, 2]:
                        kalman_predictions[f'step_{horizon}'] = {
                            'state': np.array([0.1, 0.05]),  # Default state prediction
                    ...
                    if k > 0 and historical_portfolios:
                        # Use historical data for more accurate predictions
                        for sol in pareto_frontier:
                            if hasattr(sol.P, 'kalman_state') and sol.P.kalman_state:
                                try:
                                    kalman_pred = self.n_step_predictor.kalman_n_step_prediction(sol.P.kalman_state, h)
                                    kalman_predictions[f'step_{h}'] = kalman_pred[f'step_{h}']
                                except:
                                    pass  # Use default predictions if Kalman fails
                        
                        try:
                            dirichlet_pred = self.n_step_predictor.dirichlet_n_step_prediction(
                                np.ones(len(historical_data.columns)), historical_portfolios, h)
                            dirichlet_predictions[f'step_{h}'] = dirichlet_pred[f'step_{h}']
                        except:
                            pass  # Use default predictions if Dirichlet fails
                    
                    # Calculate benchmarks
                    benchmarks = self.benchmark_calculator.calculate_all_benchmarks(historical_data.values)
                    ...
                    for dm in decision_makers:
                        ...
                        coherence = self.calculate_coherence(portfolio_weights, population_weights)
                        
                        # Update decision maker history
                        dm.update_history(selected_portfolio, new_wealth, period_roi,
                                        transaction_cost, coherence, hv_data.get('expected_hypervolume', {}),
                                        hv_data.get('conditional_hypervolume', {}))
                ...
                run_results[f'K{k}_h{h}'] = {
                    'decision_makers': {dm.name: {
                        ...
                        'expected_hypervolume_avg': self._calculate_avg_expected_hypervolume(dm.expected_hypervolume_history),
                        'conditional_hypervolume_avg': self._calculate_avg_conditional_hypervolume(dm.conditional_hypervolume_history)
                    } for dm in decision_makers},
    ...
    def _calculate_avg_expected_hypervolume(self, hv_history: List[Dict]) -> float:
        """Calculate average expected hypervolume across all periods"""
        if not hv_history:
            return 0.0
        ...
        for period_hv in hv_history:
            if isinstance(period_hv, dict):
                for sol_data in period_hv.values():
                    if isinstance(sol_data, dict) and 'expected_hypervolume' in sol_data:
                        total_hv += sol_data['expected_hypervolume']
                        count += 1
        
        return total_hv / count if count > 0 else 0.0

    def _calculate_avg_conditional_hypervolume(self, hv_history: List[Dict]) -> float:
        """Calculate average conditional expected hypervolume across all periods"""
        if not hv_history:
            return 0.0
        ...
        for period_hv in hv_history:
            if isinstance(period_hv, dict) and 'conditional_expected_hypervolume' in period_hv:
                total_hv += period_hv['conditional_expected_hypervolume']
                count += 1
        
        return total_hv / count if count > 0 else 0.0

    def run_enhanced_experiment(self):
        """Run the enhanced experiment with multiple runs"""
        logger.info("Starting Enhanced FTSE ASMS-EMOA Experiment V2")
        ...
        # Load data
        returns_data = self.load_ftse_data(start_date, end_date)
        ...
        logger.info("Enhanced experiment V2 completed!")
    ...
    def generate_comprehensive_report(self):
        ...
        report = []
        report.append("# Enhanced FTSE ASMS-EMOA Experiment V2 Report")
        ...
        # Save report
        with open('enhanced_ibovespa_experiment_v2_report.md', 'w') as f:
            f.write('\n'.join(report))
        
        logger.info("Comprehensive report generated: enhanced_ibovespa_experiment_v2_report.md")

    def create_comprehensive_visualizations(self, all_run_results: List[Dict]):
        """Create comprehensive visualizations"""
        # Create output directory
        os.makedirs('enhanced_ibovespa_results_v2', exist_ok=True)
        ...
        logger.info("Comprehensive visualizations saved to enhanced_ibovespa_results_v2/")

    def _plot_wealth_accumulation(self, all_run_results: List[Dict]):
        """Plot wealth accumulation curves"""
        fig, axes = plt.subplots(4, 2, figsize=(15, 20))
        fig.suptitle('Wealth Accumulation Curves Across All Runs', fontsize=16)
        ...
        plt.tight_layout()
        plt.savefig('enhanced_ibovespa_results_v2/wealth_accumulation_curves.png', dpi=300, bbox_inches='tight')
        plt.close()

    def _plot_performance_comparison(self):
        """Plot performance comparison across runs"""
        fig, axes = plt.subplots(2, 2, figsize=(15, 12))
        fig.suptitle('Performance Comparison Across Runs', fontsize=16)
        ...
        for idx, (metric, metric_name) in enumerate(zip(metrics, metric_names)):
            ...
            x_pos = np.arange(len(k_values) * len(h_values))
            width = 0.25
        ...
        plt.tight_layout()
        plt.savefig('enhanced_ibovespa_results_v2/performance_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()

    def _plot_expected_hypervolume_analysis(self):
        """Plot expected hypervolume analysis"""
        fig, axes = plt.subplots(2, 2, figsize=(15, 12))
        fig.suptitle('Expected Hypervolume Analysis', fontsize=16)
        ...
        ax3 = axes[1, 0]
        # Add statistical significance analysis here if needed
        
        # Performance vs Hypervolume correlation
        ax4 = axes[1, 1]
        # Add correlation analysis here if needed
        
        plt.tight_layout()
        plt.savefig('enhanced_ibovespa_results_v2/expected_hypervolume_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()

def main():
    """Main function to run the enhanced experiment"""
    experiment = EnhancedFTSEExperimentV2()
