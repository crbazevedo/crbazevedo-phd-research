#!/usr/bin/env python3
"""
Fixed FTSE Data Downloader using yfinance with certifi
Handles SSL certificate issues properly
"""
...
def download_ftse_data(start_date="2005-01-01", end_date=None, period="10y"):
    """Download FTSE 100 data using yfinance with proper SSL handling"""
    ...
    try:
        ...
        ftse_data = yf.download("^FTSE", start=start_date, end=end_date, session=s, progress=False)
...
def download_ftse_components(start_date="2015-01-01", end_date=None, max_assets=30):
    """Download FTSE 100 component stocks with proper SSL handling"""
    ...
    for i, symbol in enumerate(ftse_symbols[:max_assets]):
        try:
            ...
            # Download stock data with session
            stock_data = yf.download(symbol, start=start_date, end=end_date, session=s, progress=False)
...
def test_ftse_download():
    ...
    try:
        ...
        df = yf.download("^FTSE", period="6mo", interval="1d", session=s, progress=False)
